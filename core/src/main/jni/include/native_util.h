...
#define LSP_NATIVE_METHOD(className, functionName, signature)                \
  { #functionName,                                                       \
    signature,                                                           \
    _NATIVEHELPER_JNI_MACRO_CAST(void*) (Java_com_posed_lspd_nativebridge_## className ## _ ## functionName) \
  }
#endif
...
#define LSP_DEF_NATIVE_METHOD(ret, className, functionName, ...)                \
  extern "C" ret Java_com_posed_lspd_nativebridge_## className ## _ ## functionName (JNI_START, ##  __VA_ARGS__)
#endif
...
inline std::string GetNativeBridgeSignature() {
    ...
    static auto signature = obfs_map.at("com.posed.lspd.nativebridge.");
    return signature;
}
